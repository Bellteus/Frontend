...
function resumenPorAgente(audios: AudioWithAnalysis[]) {
  const agentes: Record<string, AudioWithAnalysis[]> = {};
  audios.forEach(a => {
    const id = a.IdEmpleado || 'Sin ID';
    if (!agentes[id]) agentes[id] = [];
    agentes[id].push(a);
  });

  return Object.entries(agentes).map(([id, registros]) => {
    const nombre = registros[0]?.NombreEmpleado || 'Sin nombre';
    ...
    const clientesUnicos = new Set(registros.map(r => r.Cliente)).size;
    return {
      id,
      nombre,
      totalLlamadas,
...
const coloresAgentes = [
  "#6366f1", "#ef4444", "#10b981", "#eab308", "#3b82f6",
  "#be185d", "#f43f5e", "#34d399", "#f59e42", "#0ea5e9"
];
...
const DashboardAgente: React.FC = () => {
  ...
  const pieDataAgentes = {
    ...
    datasets: [{
      data: agentesResumen.map(a => a.totalLlamadas),
      backgroundColor: coloresAgentes,
    }]
  };

  // BAR HORIZONTAL - Satisfacción promedio por agente
  const barDataSatisfaccionPorAgente = {
    labels: agentesResumen.map(a => a.nombre),
    datasets: [{
      label: "Satisfacción promedio (%)",
      data: agentesResumen.map(a => a.satisfaccionPromedio),
      backgroundColor: coloresAgentes,
      borderRadius: 8,
      barThickness: 28,
      borderWidth: 1,
    ...
    datasets: topAgentes.map((agente, idx) => {
      const llamadasPorDia = fechasUnicas.map((fecha, i) => {
        ...
        return audios.filter(a =>
          a.IdEmpleado === agente.id && a.FechaHoraInicio.startsWith(fecha)
        ).length;
      });
      return {
        label: agente.nombre,
        data: llamadasPorDia,
        borderColor: coloresAgentes[idx % coloresAgentes.length],
        backgroundColor: coloresAgentes[idx % coloresAgentes.length] + "22",
        tension: 0.55,
        fill: true,
        pointRadius: 3,
        pointHoverRadius: 6,
  ...
  if (loading) return (
    <div className="min-h-screen w-full bg-[#f8f9ff] font-inter px-2 sm:px-4 md:px-8 lg:px-12 xl:px-20 2xl:px-32 py-3 flex flex-col min-h-screen">
      ...
      <div className="flex-1 flex items-center justify-center mt-8">
        <div className="p-16 text-center text-lg font-medium">Cargando datos de agentes...</div>
      </div>
    </div>
  );
  if (error) return (
      ...
      ">
        ...
        <div className="flex flex-col xl:flex-row gap-7 mb-10 w-full">
          <div className="bg-white rounded-xl shadow-md p-6 flex-1 flex flex-col items-center min-w-[260px]">
            <h3 className="text-[#3341a5] font-bold text-base sm:text-lg mb-3 text-center">Distribución de llamadas por agente</h3>
            <div className="w-full h-64 max-w-[420px]">
              <Pie data={pieDataAgentes} options={{
                plugins: {
          ...
          <div className="bg-white rounded-xl shadow-md p-6 flex-[2] flex flex-col items-center min-w-[260px]">
            ...
            <div className="w-full h-72">
              <Bar data={barDataSatisfaccionPorAgente} options={{
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-10">
          <h3 className="text-[#3341a5] font-bold text-base sm:text-lg mb-5 text-center">Evolución diaria de llamadas por agente</h3>
          <div style={{ minHeight: 320, maxHeight: 500 }}>
            <Line data={lineDataAgentes} options={{
              responsive: true,
              plugins: {
        ...
        <div className="bg-white rounded-xl shadow-md p-6 w-full min-w-0 mb-0">
          <h2 className="text-[#2c3e8f] text-lg sm:text-xl font-bold mb-5">Resumen de desempeño por agente</h2>
          <div className="overflow-x-auto">
            <table className="w-full border-separate border-spacing-y-2 bg-white mt-4 min-w-[480px]">
              <thead>
                <tr>
                  <th className="bg-[#f8f9ff] text-[#2c3e8f] font-semibold text-sm sm:text-base px-4 py-3 text-left">Agente</th>
                  ...
                  <th className="bg-[#f8f9ff] text-[#2c3e8f] font-semibold text-sm sm:text-base px-4 py-3 text-left">Clientes</th>
                </tr>
              </thead>
              <tbody>
                {agentesResumen.map((agente, idx) => (
                  <tr key={agente.id} className="bg-white hover:bg-[#eaf1fb] cursor-pointer transition">
                    ...
                    <td className="px-4 py-3">{agente.satisfaccionPromedio}%</td>
                    <td className="px-4 py-3">{agente.tmo} min</td>
                    <td className="px-4 py-3">{agente.clientes}</td>
                  </tr>
                ))}
